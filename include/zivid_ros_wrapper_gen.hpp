...
{
    static_assert("The default getTypeNameFromZividState type was called. You will need to specify type conversion of this value. (Standar ros message types required)");
    return "";
}
template<>
...
public:
    ...
    {
        static_assert(AssertPair<StateType, SRVType>::value, "No conversion is specified between this srv type to state type");
    }
    static SRVType stateValueToSrvValue(const StateType& state_value)
    {
        static_assert(AssertPair<StateType, SRVType>::value, "No conversion is specified between this state type to srv type");
    }
    static std::string getSrvValueType(void)
    {
        static_assert(AssertPair<StateType, SRVType>::value, "No srv type string is specified for this state type to srv type");
        return "";
    }
};
template <>
...
{
public:
    static bool srvValueToStateValue(const uint8_t& srv_value){ return srv_value; }
    static uint8_t stateValueToSrvValue(const bool& state_value){ return state_value; }
    static std::string getSrvValueType(void){ return "bool"; }
};
template<>
class StateSrvValueConversion<double, double>
{
public:
    static double srvValueToStateValue(const double& srv_value){ return srv_value; }
    static double stateValueToSrvValue(const double& state_value){ return state_value; }
    static std::string getSrvValueType(void){ return "float64"; }
};
...
public:
    ...
    {
        static_assert(AssertPair<SettingsType, ConfigType>::value, "TODO false A config to settings value conversion must be added for this setting type. A settings type must be converted to either int, bool, double or std::string");
    }
    static ConfigType settingsValueToConfigValue(const SettingsType& settings_value)
    {
        static_assert(AssertPair<SettingsType, ConfigType>::value, "TODO false A settings to config value conversion must be added for this setting type. It must be converted from either int, bool, double or std::string");
    }
    static std::string getConfigValueType(void)
    {
        static_assert(AssertPair<SettingsType, ConfigType>::value , "TODO false A config typename must be specified as a string wich is either bool_t, int_t, double_t or str_t");
        return "";
    }
};
template <>
...
{
public:
    static bool configValueToSettingsValue(const bool& config_value){ return config_value; }
    static bool settingsValueToConfigValue(const bool& settings_value){ return settings_value; }
    static std::string getConfigValueType(void) { return "bool_t"; }
};
template <>
class ConfigSettingsValueConversion<double, double>
{
public:
    static double configValueToSettingsValue(const double& config_value){ return config_value; }
    static double settingsValueToConfigValue(const double& settings_value){ return settings_value; }
    static std::string getConfigValueType(void){ return "double_t"; }
};
template <>
...
public:
    ...
    static std::string getConfigValueType(void){ return "int_t"; }
};
...
public:
    ...
    {
        static_assert(AssertValue<ConfigType>::value, "No config value reader found for this config type. The currently only supported types are bool, int, double and string");
    }
};
...
public:
    static SettingsType getConfigValueFromSetting(const std::string& key, const dynamic_reconfigure::Config& config)
    {
        ...
        throw std::invalid_argument("The key argument " + key + " is not found in the config message. Is both ROS wrapper and Zivid API up to date?");
    }
};
template <typename SettingsType>
    ...
    static SettingsType getConfigValueFromSetting(const std::string& key, const dynamic_reconfigure::Config& config)
    {
        for(const auto& string_config : config.strs)
        {
            if(string_config.name == key)
                return ConfigSettingsValueConversion<SettingsType, std::string>::configValueToSettingsValue(string_config.value);
        }
        throw std::invalid_argument("The key argument " + key + " is not found in the config message. Is both ROS wrapper and Zivid API up to date?");
    }
};
...
SettingsType getConfigValueFromString(const std::string& key, const dynamic_reconfigure::Config& config)
{
    if constexpr(std::is_same_v<SettingsType, bool>)
        ...
        static_assert(AssertValue<SettingsType>::value, "Setting to config type convertion is not specified. You must specify which config type each settings type should be converted to. Options are: bool, int, double and string");
}
