...
namespace {
  ...
  std::string getRangeValueText(const ZividSetting& zividSetting)
  {
    ValueType default_value =
        ConfigSettingsValueConversion<typename ZividSetting::ValueType, ValueType>::settingsValueToConfigValue(zividSetting.value());
    ValueType min_value =
  ...
  std::string generateSettingCode(const ZividSetting& zividSetting, const std::string& parent_variable)
  {
    ...
    {
      static_assert(AssertValue<typename ZividSetting::ValueType>::value, "A default text and type name is required for type "
  ...
  template <typename ZividSetting>
  std::string generateGroupCode(const ZividSetting& zividSetting, const std::string& parent_variable, const std::string& groupVariable)
  {
    std::stringstream ss;
  ...
  std::string frameSettingsConfig(const ZividSetting& zividSetting, const std::string& parent_variable)
  {
    ...
    else if constexpr (ZividSetting::isContainer)
    {
      ...
      zividSetting.forEach([&](const auto& member) {
        ss << frameSettingsConfig(member, parent_variable);
      });
      return ss.str();
    } else {
  ...
  template <typename ZividSetting>
  std::string generalSettingsConfig(const ZividSetting& zividSetting, const std::string& parent_variable, bool anyParentNodeIsGlobal = false)
  {
