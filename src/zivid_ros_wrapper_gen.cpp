...
{
    ValueType default_value = ConfigSettingsValueConversion<typename Target::ValueType, ValueType>::settingsValueToConfigValue(target.value());
    ValueType min_value = ConfigSettingsValueConversion<typename Target::ValueType, ValueType>::settingsValueToConfigValue(target.range().min());
    ValueType max_value = ConfigSettingsValueConversion<typename Target::ValueType, ValueType>::settingsValueToConfigValue(target.range().max());
...
std::string generateSettingCode(const Target& target, const std::string& parent_variable)
{
    ...
    {
        default_text = ConfigSettingsValueConversion<bool, bool>::settingsValueToConfigValue(target.value()) ? "True" : "False";
    ...
    {
        static_assert(AssertValue<typename Target::ValueType>::value, "A default text and type name is required for type Target::ValueType");
    }

    return_string = parent_variable + ".add(\"" +
        setting_name + "\", "
        + type_name + ", "
        + some_number + ", "
        + "\"" + description + "\", "
...
void writeStateSrvFile(const Target& target)
{
    ...
    srv_file << "---" << std::endl;
    srv_file << type_name << " " << "value" << std::endl;
    srv_file.close();
}
...
template <typename Target>
std::string generateSrvServerSetup(const Target& target, const std::string& node_handle_name, const std::string zivid_camera_name, const std::string& server_list_name)
{
    ...
    std::string callback_variable_text =
        "boost::function<bool(" +
        callback_template_req_type + ", " +
        callback_template_res_type + ")> " +
        function_callback_variable_name + " = boost::bind(& " +
        callback_function_name + ", _1, _2, " + zivid_camera_name + ");";
    ...
    std::string srv_setup_string =
        info_string + ";\n\t" +
        callback_variable_text + "\n\t" +
        server_list_name + ".push_back(" +
        node_handle_name + ".advertiseService(\"" +
        service_path + "\", " +
...
void writeZividSrvHeader(const std::string& file_name)
{
    ...
    std::string main_function_name =
        "void setupStatusServers(ros::NodeHandle&" + node_handle_name +
